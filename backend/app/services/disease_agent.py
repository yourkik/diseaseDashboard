...
def get_disease_map_data_from_agent(disease_keyword, base_data_str=""):
    if not PROJECT_ENDPOINT:
    ...
    try:
        ...
        thread = project_client.agents.threads.create()
        prompt = f'''최근 한국의 {disease_keyword} 유행 관련 최신 뉴스를 찾아보고, 각 지역별(서울, 경기, 부산 등) 위험도(High, Medium, Low) 및 전파 방향을 분석하세요.
